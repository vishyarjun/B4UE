...
export async function POST(request: Request) {
  ...
  try {
    console.log('Food scan proxy: Starting request to external API');
    ...
    if (!response.ok) {
      ...
      console.error('Food scan proxy: External API error', {
    ...
    console.log('Food scan proxy: Successfully processed request');
    return NextResponse.json(data);
  } catch (error) {
    console.error('Food scan proxy: Detailed error:', {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to process food image' },
      { status: 500 }
    );
  }
}
